...
export async function planAndDraftActions(
  ...
  let targetNote = findNoteFromMessage(notes, matchableText, context.noteId);
  const titleForCreate = createIntent?.title ?? null;
  let focusTitle: string | null = targetNote?.title ?? titleForCreate ?? null;
  let quickLogUsed = false;

  if (!createIntent && !wantsSources) {
    const quickLog = detectQuickLogIntent(text, notes);
    if (quickLog) {
      actions.push(...quickLog.actions);
      quickLogUsed = true;
      if (!targetNote && quickLog.anchorNote) {
        targetNote = quickLog.anchorNote;
      }
      if (quickLog.focusTitle) {
        focusTitle = quickLog.focusTitle;
      } else if (!focusTitle && quickLog.anchorNote) {
        focusTitle = quickLog.anchorNote.title;
      }
    }
  }
  ...
  if (!createIntent && !quickLogUsed && !targetNote && actions.length === 0) {
  ...
  const reply = craftReply(message, actions, focusTitle ?? targetNote?.title ?? titleForCreate);
...
type QuickLogResult = {
  actions: DraftAction[];
  anchorNote?: { id: string; title: string };
  focusTitle?: string;
};
...
function detectQuickLogIntent(message: string, notes: AgentContext["notes"]): QuickLogResult | null {
  ...
  const hasActionVerb = /(делал(?:и)?|прошл(?:и|о)|разбирал(?:и)?|считали|обсуждал(?:и)?|трениров(?:ался|ались)|работал(?:и)?|изучали|катал(?:ся|ись)|поехал(?:и)?|гонял(?:и)?|ходил(?:и)?|вышл(?:и|а)|двигал(?:ись)?)/i.test(text);
  const hasContext = hasKeywordContext || (hasActionVerb && (hasDate || hasNumber));
  ...
  const anchorNote = bestMatch && bestMatch.score >= 0.6 ? bestMatch.note : undefined;
  actions.push({
    type: "create_note",
    title: childTitle,
  ...
  const focusTitle = anchorNote?.title ?? baseTitle;
