...
export async function reindexNote(noteId: string, content: string) {
  await reindexNoteWithClient(db, noteId, content);
}

export async function reindexNoteWithClient(client: typeof db | any, noteId: string, content: string) {
  await client.delete(noteChunks).where(eq(noteChunks.noteId, noteId));
  ...
  await client.insert(noteChunks).values(rows);
}
